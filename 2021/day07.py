...
# sort the array
# start with total assuming first element
# traverse array till you find an increasing number
# then you bump (idx + 1)


from collections import Counter

def find_most_common(vals):
    cc = Counter(vals)
    most_common_vals = cc.most_common(1)
    print('most_common_vals', most_common_vals)
    return [val for val, _counts in most_common_vals]


def get_next_unique_val(vals):
    curr_val = None
...
def get_total_for_val(baseline, counter, cost_function):
    return sum(
        cost_function(val, baseline) * count
        for val, count in counter.items()
    )


def problem_one(vals):
    # don't need to sort since we use a counter
    # vals = sorted(vals)
    ...
    # get totals based on unique values
    for idx, val in get_next_unique_val(vals):
        new_total = get_total_for_val(
            val,
            counter,
            cost_function=lambda val, baseline: abs(val - baseline)
        )
        if new_total < smallest_total:
            # print('new smallest_total', smallest_total, val)
            smallest_total = new_total

    return smallest_total


def old_problem_one(vals):
    most_common_vals = find_most_common(vals)

    print('most_common_vals', most_common_vals)

    totals = []
    for idx, baseline in enumerate(most_common_vals):
        totals.append(0)
        for val in vals:
            if val == baseline:
                continue

            totals[idx] += abs(val - baseline)

    minimum = float('inf')
    minimum_val = None
    for idx, val in enumerate(most_common_vals):
        if totals[idx] < minimum:
            minimum = totals[idx]
            minimum_val = val

    print(minimum_val, minimum)
    return minimum
...
def problem_two(vals):
    ...
    for val in range(max(vals)):
        new_total = get_total_for_val(
            val,
            counter,
            cost_function=lambda val, baseline: sum(range(1, abs(val - baseline) + 1))
        )
        if new_total < smallest_total:
            print('new smallest_total', smallest_total, val)
            smallest_total = new_total
...
if __name__ == '__main__':
    ...
    run(problem_one, get_input(q1), 37)
    run(problem_one, get_input(q2), 340987)

    run(problem_two, get_input(q1), 168)
    run(problem_two, get_input(q2), 96987874)
