...
# all characters of 7 are in 9 but not in 6


from collections import defaultdict


def process_numbers(number_strings, mapping=lengths_map):
    processed = {}

    leftovers = defaultdict(list)
    for num in number_strings:
        size = len(num)
        if len(mapping[size]) == 1:
            processed[mapping[size][0]] = num
        else:
            leftovers[size].append(num)

    four_set = set(processed[4])
    zero_or_six = []
    nine = None
    for num in leftovers[6]:
        if len(set(num) - four_set) == 2:
            nine = num
        else:
            zero_or_six.append(num)

    zero, six = None, None
    if len(set(zero_or_six[0]) - set(processed[1])) == 4:
        # must be zero
        zero, six = zero_or_six
    else:
        six, zero = zero_or_six
    ...
    # 2, 3, 5 remaining
    two_and_three = []
    for num in leftovers[5]:
        if len(set(num) - set(six)) == 0:
            processed[5] = num
        else:
            two_and_three.append(num)

    # for num in two_and_three:
    two, three = None, None
    if set(two_and_three[0]) - set(nine):
        # must be two if anything left
        two, three = two_and_three
    else:
        three, two = two_and_three

    processed[2] = two
    processed[3] = three

    print(processed)
    return {frozenset(val): key for key, val in processed.items()}


def translate_number(mapping, number_string):
    val = mapping[frozenset(number_string)]
    return val
...
def problem_one(lines):
    ...
    def process_line(vals):
        ...
        # return "".join(translate_number(processed_numbers, i) for i in output)
        simple_number_count = 0
        for i in output:
            if translate_number(processed_numbers, i) in [1, 4, 7, 8]:
                simple_number_count += 1

        return simple_number_count
...
def problem_two(lines):
    ...
    def process_line(vals):
        ...
        return sum(
            10 ** (3 - idx) * translate_number(processed_numbers, i)
...
if __name__ == '__main__':
    run(problem_one, get_input(q1), 26)
    run(problem_one, get_input(q2), 342)

    run(problem_two, get_input(q1), 61229)
