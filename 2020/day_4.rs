...
// def valid_number(value, low_range, high_range):
//     if (not len(value) == 4) and (not value.isnumeric()):
//         return False

//     value = int(value)
//     return low_range <= value <= high_range
...
//     if 'cid' in passport:
//         passport.pop('cid')
//     north_pole_keys = set(('byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'))

//     for key in north_pole_keys:
//         if not validate(key, passport.get(key)):
//             return False

//     return True
//     # print(passport.keys(), north_pole_keys)
//     # return set(passport.keys()) == north_pole_keys

// def parse_passport_details(lines):
//     valid_passports = []
//     curr_passport = {}
...
fn parse_passport_details(lines: &Vec<&str>) -> Vec<HashMap<String, String>> {
  let valid_passports: Vec<HashMap<String, String>> = vec![];
  let mut curr_passport: HashMap<String, String> = HashMap::new();

  for line in lines.iter() {
    if line.len() == 0 {
      println!("empty {:?}", line);
      if check_passport(&curr_passport) {
        valid_passports.push(curr_passport);
      }
    ...
    for item in line.split(" ").collect::<Vec<&str>>() {
      let [key, value] =
      let key_val = item.split(":").collect::<Vec<&str>>();

      if key_val.len() == 2 {
        println!("hie");
      }
      // {
      //    => curr_passport.insert(key, value),
      //   whatevs => println!("that shouldn't have happened...{:?}", whatevs)
      // }
    }
    println!("curr_passport {:?}", curr_passport);
  }

  return valid_passports;
}

fn get_valid_passports_count(lines: &Vec<&str>) -> usize {
  return parse_passport_details(&lines).len();
}
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719
  ...
  println!("valid passports: {:?}", get_valid_passports_count(&vals));
