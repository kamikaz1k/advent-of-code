...

{
    ...
    'muted yellow': {
        'shiny gold bags': 2,
        'faded blue bags': 9
    }, 'shiny gold': {'dark olive bag': 1, 'vibrant plum bags': 2}, 'dark olive': {'faded blue bags': 3, 'dotted black bags': 4}, 'vibrant plum': {'faded blue bags': 5, 'dotted black bags': 6}, 'faded blue': 0, 'dotted black': 0}


# find a bag that will accept you

# find what other bags you have to add

# after you add a bags, move up a level
...
def wrap_bag(
    ...
    if current_bag not in acceptable_wrappers:
        return

    for acceptable in acceptable_wrappers[current_bag]:
        # this commented code is for tracking totals
        # of each bag across the entire "stack"

        # differences = {}
        # no_dice = False
        # for bag, amt in rules[acceptable].items():
        #     if total_bags[bag] > amt:
        #         no_dice = True
        #         break

        #     differences[bag] = amt - total_bags[bag]
        #     total_bags[bag] += differences[bag]

        # if no_dice:
        #     for bag, amt in differences.items():
        #         total_bags[bag] -= amt
        #     continue

        outer_most_bags.add(acceptable)
        wrap_bag(
            rules,
            acceptable_wrappers,
            acceptable,
        ...
        # pop the stack/totals for the next loop
        # for bag, amt in rules[acceptable].items():
        #     total_bags[bag] -= differences[bag]
...
def go_deep(
    ...
    *args
    ...
    totals = 0
    for curr_bag, amts in rules[current_bag].items():
        totals += amts
        res = go_deep(rules, acceptable_wrappers, curr_bag)
        totals += (res * amts)
...
def parse_rules(rules):
    bags = {}
    acceptable_wrapper = defaultdict(list)
    for line in rules:
        ...
        for contain in contains:
            ...
            if aye[0] == 'no':
                ...
                count = int(aye[0])
                wrappable = ' '.join(aye[1:]).replace(" bags", "").replace(" bag", "")
                bags[container][wrappable] = count
                acceptable_wrapper[wrappable].append(container)

    return bags, acceptable_wrapper


def problem_one(val):
    rules, acceptable = parse_rules(val)

    print("total_bags", len(rules), len(acceptable))
    total_bags = {key: 0 for key in rules.keys()}

    total_bags["shiny gold"] = 0
    outer_most_bags = set()

    wrap_bag(rules, acceptable, "shiny gold", total_bags, outer_most_bags)

    print('outer_most_bags', outer_most_bags)
    return len(outer_most_bags)
...
def problem_two(val):
    ...
    result = go_deep(rules, acceptable, "shiny gold")

    print('go_deep', result)
    return result
...
if __name__ == '__main__':
  ...
  # 242 is answer
  print('result for problem_one', problem_one(values))

  # 176035 is answer
  print('result for problem_two', problem_two(values))
