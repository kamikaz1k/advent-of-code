...
def is_valid_pass(value):
    count_range, check_char, password = value.split(' ')
    check_char = check_char[:-1]
    count_range = count_range.split('-')
    count_range_low, count_range_high = int(count_range[0]), int(count_range[1])
    ...
    return count_range_low <= count <= count_range_high

def is_valid_pass2(value):
    check_idx, check_char, password = value.split(' ')
    check_char = check_char[:-1]
    check_idx = check_idx.split('-')
    check_idx_low, check_idx_high = int(check_idx[0]), int(check_idx[1])

    check_value_low = False
    if check_idx_low <= len(password):
        check_value_low = password[check_idx_low - 1] == check_char

    check_value_high = False
    if check_idx_high <= len(password):
        check_value_high = password[check_idx_high - 1] == check_char

    result = check_value_low != check_value_high

    expr = 'valid' if result else 'invalid'
    # print(f'{password} is {expr}')
    return result

def get_valid_pass_count(vals, type):
    if type == 2:
        func = is_valid_pass2
    if type == 1:
        func = is_valid_pass
...
if __name__ == '__main__':
    print('part 1 valid count:', get_valid_pass_count(vals, 1) == 640)
    print('part 2 valid count:', get_valid_pass_count(vals, 2) == 472)
