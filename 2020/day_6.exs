defmodule AdventOfCode.Y2020.Day06 do
  ...
  b
  ''' |> to_string() |> String.trim() # otherwise it is a charlist
  ...
  def to_set(answers) do
    to_set(MapSet.new(), answers)
  end
  ...
  def to_set(set, [answer | remaining_answers]) do
    answer
    |> String.graphemes()
    |> Enum.reduce(set, fn char, set ->
      MapSet.put(set, char)
    end)
    |> to_set(remaining_answers)
  end

  def get_counts(vals, "set") do
    vals
    |> String.split("\n\n")
    # |> IO.inspect(label: "split into groups")
    |> Enum.map(&String.split(&1, "\n"))
    # |> IO.inspect(label: "split groups into people")
    |> Enum.map(&to_set/1)
    # |> IO.inspect(label: "unique answers for groups")
    |> Enum.concat()
    |> length()
  end

  def to_intersection([answer | remaining_answers]) do
    answer
    |> String.graphemes()
    |> MapSet.new()
    |> to_intersection(remaining_answers)
  end

  def to_intersection(set, []), do: set |> MapSet.to_list()

  def to_intersection(set, [answer | remaining_answers]) do
    answer
    |> String.graphemes()
    |> MapSet.new()
  ...
  def get_counts(vals, "intersection") do
    vals
    |> String.split("\n\n")
    # |> IO.inspect(label: "split into groups")
    |> Enum.map(&String.split(&1, "\n"))
    # |> IO.inspect(label: "split groups into people")
    |> Enum.map(&to_intersection/1)
    # |> IO.inspect(label: "unique answers for groups")
    |> Enum.concat()
    |> length()
  end

  def check_expected(actual, expected, test_name) do
    case expected == actual do
      false -> IO.puts("expected #{expected} but got #{actual}")
      _ -> IO.puts("#{test_name} is gucci mayne")
    end
  end

  def run() do
    IO.puts("### Starting Day 6\n")

    get_counts(@test_vals, "set")
    |> check_expected(11, "test get_counts/set for @test_vals")

    get_counts(@vals, "set")
    |> check_expected(6714, "test get_counts/set for @vals")

    get_counts(@test_vals, "intersection")
    |> check_expected(6, "test get_counts/intersection for @test_vals")

    get_counts(@vals, "intersection")
    |> check_expected(3435, "test get_counts/intersection for @vals")

    IO.puts("\n### Done")
  end
end

# run script by running command `elixir day_6.exs`
AdventOfCode.Y2020.Day06.run()
